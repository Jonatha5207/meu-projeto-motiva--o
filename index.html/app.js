...
function render() {
  ...
  if (currentView === 'community') loadCommunityData();
...
function renderCommunity() {
  ...
  return `<section class="screen community-screen">${header('Comunidade', 'COMUNIDADE')}<p class="lead">Você não precisa fazer tudo sozinho. Veja quem está treinando agora e conecte-se de verdade.</p>${renderPulseStrip()}${incoming.length ? `<div class="section-title"><h3>Pedidos de conexão</h3></div><div class="community-list">${incoming.map(item => `<article class="person-card request-card"><div class="person-avatar">${escapeHtml((item.user?.name || '?')[0])}</div><div class="person-main"><div class="person-line"><div><h3>${escapeHtml(item.user?.name || 'Alguém')}</h3><p class="small">quer treinar com você</p></div></div><div class="request-actions"><button class="primary" data-respond-connection="${item.id}" data-accept="true">Aceitar</button><button class="secondary" data-respond-connection="${item.id}" data-accept="false">Recusar</button></div></div></article>`).join('')}</div>` : ''}<div class="section-title"><div class="stat-line"><h3>Para seu próximo treino</h3><span class="status-pill">${escapeHtml(data.profile.activity || '')}</span></div></div><div class="community-list">${people.length ? people.map(person => `<article class="person-card"><div class="person-avatar">${escapeHtml(person.name[0])}</div><div class="person-main"><div class="person-line"><div><h3>${escapeHtml(person.name)}</h3><p class="small">${escapeHtml(person.activity || 'Atividade livre')}${person.time ? ' · ' + escapeHtml(person.time) : ''}</p></div>${communityPresenceBadge(person)}</div>${communityConnectButton(person)}</div></article>`).join('') : '<p class="small">Ainda não há outras pessoas cadastradas com essa atividade. Volte em breve.</p>'}</div>${accepted.length ? `<div class="section-title"><h3>Suas conexões</h3></div><div class="community-list">${accepted.map(item => `<article class="person-card"><div class="person-avatar">${escapeHtml((item.user?.name || '?')[0])}</div><div class="person-main"><div class="person-line"><div><h3>${escapeHtml(item.user?.name || 'Conexão')}</h3><p class="small">${item.online ? 'online agora' : 'offline'}</p></div></div><button class="secondary partner-button" data-open-chat="${item.id}">Conversar</button></div></article>`).join('')}</div>` : ''}<div class="community-note"><strong>Encontro seguro primeiro.</strong><span>Compartilhe só o necessário. O app não mostra seu telefone nem sua localização exata.</span></div></section>`;
}
...
function renderMap() { const points = meetingPointsForUser(); return `<section class="screen map-screen">${header('Pontos de encontro', 'REDE DE PRESENÇA')}<p class="lead">Encontre pessoas da sua modalidade em locais públicos. Você escolhe quando e com quem conversar.</p><div class="map-safety"><span class="safety-mark">✓</span><div><strong>Privacidade primeiro</strong><p>Mostramos pontos públicos e interesses agrupados. Nunca sua localização exata.</p></div></div><div id="meeting-map" aria-label="Mapa com pontos públicos de encontro"></div><button class="secondary map-fit-button" data-fit-meeting-map>Ver todos os pontos no mapa</button><div class="section-title"><div class="stat-line"><h3>Para ${escapeHtml(data.profile.activity)}</h3><span class="status-pill">${points.length} pontos</span></div></div><div class="meeting-point-list">${points.map(point => { const joined = data.community.meetingPointIds.includes(point.id); const count = point.members + (joined ? 1 : 0); return `<article class="card meeting-point-card"><div class="meeting-point-head"><div class="meeting-point-icon">${sportIcon(data.profile.activity)}</div><div><h3>${escapeHtml(point.name)}</h3><p class="small">${escapeHtml(point.city)}</p></div><span class="point-distance">público</span></div><p class="meeting-point-note">${escapeHtml(point.note)}</p><div class="meeting-point-nav">${point.id !== 'qualquer-cidade' ? `<a class="nav-link" href="https://waze.com/ul?ll=${point.lat},${point.lng}&navigate=yes" target="_blank" rel="noopener">Abrir no Waze</a><a class="nav-link" href="https://www.google.com/maps/search/?api=1&query=${point.lat},${point.lng}" target="_blank" rel="noopener">Abrir no Google Maps</a>` : `<a class="nav-link" href="https://waze.com/ul?q=${encodeURIComponent(data.profile.activity || 'atividade fisica')}&navigate=yes" target="_blank" rel="noopener">Buscar no Waze perto de mim</a><a class="nav-link" href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent((data.profile.activity || 'atividade fisica') + ' perto de mim')}" target="_blank" rel="noopener">Buscar no Google Maps</a>`}</div><div class="meeting-point-meta"><span><strong>${count}</strong> interessados</span><span>${escapeHtml(point.activities.slice(0, 2).join(' · '))}</span></div><button class="${joined ? 'secondary' : 'primary'} meeting-point-button" data-meeting-point="${point.id}">${joined ? 'Você está neste ponto' : 'Quero encontrar pessoas aqui'}</button></article>`; }).join('')}</div></section>`; }
