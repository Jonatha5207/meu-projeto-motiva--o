...
function bindEvents() {
  ...
  document.querySelector('[data-onboard-next]')?.addEventListener('click', () => { if (onboardingStep === 2) { data.profile.frequency = Number(document.querySelector('#frequency').value) || 3; data.profile.days = [...document.querySelectorAll('input[name="training-days"]:checked')].map(input => input.value); data.profile.time = document.querySelector('#training-time').value || '19:00'; data.profile.scheduleByDay = Object.fromEntries(data.profile.days.map(day => [day, data.profile.time])); data.profile.duration = Number(document.querySelector('#training-duration').value) || 45; data.profile.commuteTime = document.querySelector('#commute-time').value; data.profile.transport = document.querySelector('#transport').value; } if (onboardingStep === 4) { data.profile.motivation = document.querySelector('#motivation').value; data.profile.personalizedMotivation = document.querySelector('#personalized-motivation').value.trim(); data.profile.goal = data.profile.personalizedMotivation || data.profile.motivation; data.profile.difficulty = data.profile.objection && data.profile.objection !== 'Não informado' ? data.profile.objection : 'Manter constância'; data.profile.disciplineLevel = document.querySelector('#discipline-level').value; data.profile.workStatus = document.querySelector('#work-status').value; data.profile.studyStatus = document.querySelector('#study-status').value; data.profile.hasChildren = document.querySelector('#has-children').value; } if (onboardingStep < 4) onboardingStep++; else { data.onboarded = true; awardPoints(20, 'primeiro passo'); currentView = 'summary'; } render(); });
