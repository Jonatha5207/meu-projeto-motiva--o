...
const initialData = {
  ...
  community: { requests: [], ratings: {}, checkedInDate: null, challengeJoined: false, challengeProgress: 0, goalPeriod: 'week', people: [], connections: [], pulses: [], messages: {}, activeChat: null, posts: [
...
// Um avatar distinto por modalidade (antes vários esportes compartilhavam o mesmo glifo genérico).
const sportIcons = { 'Academia': '🏋️', 'Corrida': '🏃', 'Caminhada': '🚶', 'Natação': '🏊', 'Ciclismo': '🚴', 'Crossfit': '💪', 'Dança': '💃', 'Futebol': '⚽', 'Futsal': '🥅', 'Basquete': '🏀', 'Vôlei': '🏐', 'Tênis': '🎾', 'Beach Tennis': '🏖️', 'Badminton': '🏸', 'Squash': '🎯', 'Yoga': '🧘', 'Pilates': '🤸', 'Alongamento': '🙆', 'Boxe': '🥊', 'Jiu-Jitsu': '🥋', 'Muay Thai': '🦵', 'Karatê': '🥋', 'Taekwondo': '🦶', 'MMA': '🥊', 'Skate': '🛹', 'Surf': '🏄', 'Remo': '🚣', 'Canoagem': '🛶', 'Stand up paddle': '🌊', 'Escalada': '🧗', 'Atletismo': '🏅', 'Ginástica': '🤸', 'Handebol': '🤾', 'Rugby': '🏉', 'Críquete': '🏏', 'Beisebol': '⚾', 'Softbol': '🥎', 'Hóquei': '🏒', 'Polo aquático': '🤽', 'Patinação': '⛸️', 'Triatlo': '🔺', 'Outra': '✨' };
...
function communityConnectButton(person) {
  ...
  if (state === 'connected') return `<button class="secondary partner-button" data-open-chat="${person.connection.connectionId}">Abrir conversa</button>`;
...
function renderCommunity() {
  ...
  return `<section class="screen community-screen">${header('Comunidade', 'COMUNIDADE')}<p class="lead">Você não precisa fazer tudo sozinho. Veja quem está treinando agora e conecte-se de verdade.</p>${renderPresenceStories()}${renderPulseStrip()}${incoming.length ? `<div class="section-title"><h3>Pedidos de conexão</h3></div><div class="community-list">${incoming.map(item => `<article class="person-card request-card"><div class="person-avatar">${escapeHtml((item.user?.name || '?')[0])}</div><div class="person-main"><div class="person-line"><div><h3>${escapeHtml(item.user?.name || 'Alguém')}</h3><p class="small">quer treinar com você</p></div></div><div class="request-actions"><button class="primary" data-respond-connection="${item.id}" data-accept="true">Aceitar</button><button class="secondary" data-respond-connection="${item.id}" data-accept="false">Recusar</button></div></div></article>`).join('')}</div>` : ''}<div class="section-title"><div class="stat-line"><h3>Para seu próximo treino</h3><span class="status-pill">${escapeHtml(data.profile.activity || '')}</span></div></div><div class="community-list">${people.length ? people.map(person => `<article class="person-card"><div class="person-avatar">${sportIcon(person.activity || data.profile.activity)}</div><div class="person-main"><div class="person-line"><div><h3>${escapeHtml(person.name)}</h3><p class="small">${escapeHtml(person.activity || 'Atividade livre')}${person.time ? ' · ' + escapeHtml(person.time) : ''}</p></div>${communityPresenceBadge(person)}</div>${communityConnectButton(person)}</div></article>`).join('') : '<p class="small">Ainda não há outras pessoas cadastradas com essa atividade. Volte em breve.</p>'}</div>${accepted.length ? `<div class="section-title"><h3>Suas conexões</h3></div><div class="community-list">${accepted.map(item => `<article class="person-card"><div class="person-avatar">${escapeHtml((item.user?.name || '?')[0])}</div><div class="person-main"><div class="person-line"><div><h3>${escapeHtml(item.user?.name || 'Conexão')}</h3><p class="small">${item.online ? 'online agora' : 'offline'}</p></div></div><button class="secondary partner-button" data-open-chat="${item.id}">Conversar</button></div></article>`).join('')}</div>` : ''}<div class="community-note"><strong>Encontro seguro primeiro.</strong><span>Compartilhe só o necessário. O app não mostra seu telefone nem sua localização exata.</span></div></section>`;
}
...
async function requestAiResponse(text) {
  ...
  try {
    const response = await fetch(AI_ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${authToken}` }, body: JSON.stringify({ profile: data.profile, session: data.session, messages: data.messages.slice(-12), memory: data.memory, rewards: data.rewards, live_context: liveContext, local_time: new Date().toISOString(), analysis, persona: COMPANION_PERSONA, instruction: `${COMPANION_PERSONA} Use o live_context quando fizer sentido: se houver pessoas da rede treinando agora, isso pode animar; se houver um cronômetro de treino em andamento (workout_elapsed_seconds), reconheça o esforço em curso. Se houver dor ou mal-estar importante, não incentive exercício. Ofereça uma alternativa realista quando houver falta de tempo. Não use frases genéricas de coach.` }) });
...
function bindEvents() {
  ...
  document.querySelector('[data-device-notify]')?.addEventListener('click', event => { event.currentTarget.disabled = true; event.currentTarget.textContent = 'Vamos te avisar assim que estiver pronto'; toast('Você será avisado quando relógio e NFC estiverem disponíveis'); });
